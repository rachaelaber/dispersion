...
The simulation studies indicate our LRT framework accurately detects changes in dispersion, with p-values averaging 0.5 when the effect size is zero and decreasing toward zero as the effect size increases (Fig. 1d ~\ref{fig1}). 
...
In other words, as a larger portion of incidence involved repeat infections (albeit potentially with different variants), prior immunity became a larger factor shaping epidemic dynamics, and potentially impacting the demography of burden, in particular the burden as it depends on age, and potentially particularly in children~\citep{lavine2021immunological}.
