...
Consequently, in order to estimate incidence volatility we first modeled incidence at broad spatiotemporal scales using natural splines~\citep{perperoglou2019review}. To allow for diverse shapes in the broadscale epidemic dynamics, spline modeling was conducted within a moving window
\begin{equation}
...
Explanation of the specific choices we used $J$ and $\Delta$ for our application to COVID-19 cases in US counties is described below.
...
The simulation studies indicate our LRT framework accurately detects changes in dispersion, with p-values averaging 0.5 when the effect size is zero and shrinking toward zero as the effect size increases (Fig.~\ref{fig1}). 
...
Paragraph on results shown in figure 2.
