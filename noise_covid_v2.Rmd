...
Approximately two months of data on either side of November 24th, 2020 was used in the analysis. This step was done to include a set number of "waves" of incidence in the model fit to each side of the hypothesized change. However, we acknowledge that there is a trade-off between including few waves and increasing the power of the proposed test by including more time series data. 

```{r, echo=FALSE, fig.cap="COVID-19 county-level incidence in New York surrounding Thanksgiving 2020."}
# USA case/population size data to work with 

data <- read.csv("data/confirmed_and_death_1209.csv")
data$DateTime <- as.Date(data$DateTime)

# NY dataframe for Thanksgiving analysis
data <- data[data$DateTime >= "2020-10-26" & data$DateTime <= "2020-12-24", ]
inds <- seq(as.Date("2020-10-26"), as.Date("2020-12-24"), by = "day")
days <- weekdays(inds)
inc <- aggregate(DailyNewConfirmedCases ~ countyFIPS, data, FUN = list)
inc$populations <- tapply(X = data$population, INDEX = data$countyFIPS, FUN = mean)
inc$stateFIPS <- as.numeric(substr(inc$countyFIPS, 1, 2))

# all_zero <- c()
# for (i in 1:dim(inc)[1]){
#   if (all(unlist(inc$DailyNewConfirmedCases[i]) == 0)){
#     print(i)
#     all_zero <- c(all_zero, i)
#   }
# }
# inc <- inc[-all_zero,] 
ny_df_t <- inc[inc$stateFIPS == 36,]
fl_df_t <- inc[inc$stateFIPS == 12,]
ny_fl_df_t <- rbind(ny_df_t, fl_df_t)
```
...
scaled_cases <- map(ny_df_t$DailyNewConfirmedCases, ~ as.vector(scale(.)))

ny_df_t$ch_pts <- map(scaled_cases, ~ cpts(cpt.mean(., class = TRUE, penalty = "MBIC", pen.value = 0, method = "PELT", param.estimates = TRUE)))

which(sapply(ny_df_t$ch_pts, length) == 0)
...
ny_df_t$ch_pts2 <- map(ny_df_t$DailyNewConfirmedCases, ~ cpts(cpt.meanvar(., class = TRUE, penalty = "MBIC", pen.value = 30, method = "PELT", test.stat = "Poisson", param.estimates = TRUE, minseglen = 2))) 
...
plot(ny_df_t$DailyNewConfirmedCases[[1]] ~ inds, type="l", ylim=c(0, 5000), xaxt="n", ylab="Daily Incidence", xlab="Date")
for (i in 2:dim(ny_df_t)[1]){
  lines(ny_df_t$DailyNewConfirmedCases[[i]] ~ inds, col=sample(rainbow(10)))
}
...
time <- 1:length(spline_mod.1.nb$mu)
plot(time, Y[1:(length(Y)/2)], xlab="Day Index (Segment 1)", ylab="Incidence") 
lines(time, spline_mod.1.pois$fitted.values, col="red")
lines(time, spline_mod.1.nb$mu, col="blue")
legend(10, 10, legend=c("Poisson", "NB"),
