...
Understanding patterns in infectious disease incidence is crucial for understanding epidemic dynamics and for developing effective public health responses, but traditional metrics often overlook the inherent variability in how viruses like SARS-CoV-2 (the virus that causes COVID-19) spread. Quantifying higher-frequency variation around incidence trends can elucidate important underlying processes such as transmission heterogeneity. 
We developed a method to identify shifts in the degree of dispersion (case clustering) patterns of SARS-CoV-2  within a single time series. We used negative binomial regression to account for factors that might confound the estimation of case count dispersion, allowing us to use a single parameter to infer the degree of dispersion (clustering) of cases. 
We apply the method to incidence data in 144 US counties from the January 1st, 2020 to March 23rd, 2023, and find that conspicuous shifts in dispersion occurred across counties concurrently, and that these shifts are not explained by incidence alone. 
Around incidence peaks, dispersion increased, potentially indicating that some individuals spread the virus to many more people than others. 
Additionally, as the pandemic progressed, dispersion also increased, suggesting increased differences in factors such as how people transmit the virus or how susceptible they are to the virus.
Shifts in dispersion can indicate shifts in epidemic phase as well as provide information about underlying drivers of transmission, so our method to accurately measure disperison regardless of population size and incidence provides a way to help public health officials anticipate and manage outbreaks.
...
Statistical techniques to estimate dispersion in epidemic time series at the population level are emerging.
