...
We present a method to identify shifts in population-level dispersion in incidence, so the impact of events or NPIs on superspreading dynamics can be assessed using freely available population-level data. This method may also be useful in identifying geographical areas that would benefit from targeted control strategies to reduce superspreading dynamics. Implementing and evaluating targeted control strategies has typically required detailed data on individual-level variation in transmission rates, which are often unavailable.
...
**Using county-level SARS-CoV-2 time series from New York state, we investigate dispersion in case count time series (by fitting negative binomial models) and its relationship to a hypothesized changepoint. Specifically, we estimate and compare dispersion on either side of a putative changepoint in variability (Thanksgiving 2020), in order to test the hypothesis that dispersion changes significantly**. 
...
We additionally use the full dataset to "scan" for variability breakpoints using a likelihood ratio test statistic and visualizations of a dispersion metric.
...
The resulting probabilities of rejection are tabulated below: the proportion of tests that reject is around 0.05 for those instances where the null is true (the probability of the normal variable having 0.05 of the probability density below is 0.05). Additionally, the proportion of tests that reject is high in instances where the alternative hypothesis is true. We also simulated data assuming that theta was a function of the mean of the process in order to assess the robustness of the test (Supplement).
...
![Mapping the Thanksgiving hypothesis across the US](./figures/map_difftheta_US.pdf){width=30%, height=30%}
...
In many US counties between 2020-09-27 and 2021-01-25, there is less burstiness (larger theta) in COVID-19 case counts over time. In other words, cases fluctuate less around a time-averaged trajectory later on, possibly indicating structure in the population with some groups experiencing different infection rates (overdispersed individual reproduction number), demographic/environmental stochasticity, or fast changes in propagation/reporting at the population level may have occurred earlier on. However, the trend of decreased burstiness over this period seems to be modulated by population size: among the largest population counties per state, those in the top half had an average theta change of approx. 28.95604 (less burstiness), while the small populations had an average change in theta of approx. -3.741856 (more burstiness). This means that in the large populations, dynamics became "smoother", but this didn't happen in the smaller populations, where more burstiness might be a result of cases "stuttering out". This is a key observation, as smoother and more predictable case count time series may require less surge capacity in hospitals.

We hypothesized that there would be an increase in burstiness (decrease in theta) around Thanksgiving 2020 due to people gathering and causing clusters of infection. However, we observed that in many counties, although there was more burstiness (a decrease in theta), it was a relatively small change, and not modulated by population size. The average difference between theta estimated in the window starting at 2020-11-26 and theta estimated in the window ending 2020-11-25 was -1.77944 in the top half and -1.104289 in the bottom half of population sizes. People may have avoided gathering due to personal preference, cultural norms, and other factors, contributing to less superspreading than anticipated

![Compare](./figures/compare.pdf){width=30%, height=30%}
