...
The negative binomial distribution might accurately model a time series if there is a changing process mean: for example, if the mean of a Poisson distribution itself follows a gamma distribution, the resulting distribution is negative binomial (Cook 2009). Negative binomial regression (in contrast to Poisson regression) can account for unobserved heterogeneity, time dependence in the rate of a process and contagion that all lead to overdispersion (Barron 1992).

**Using county-level SARS-CoV-2 time series, we investigate dispersion in case count time series (by fitting negative binomial models) and its relationship to a hypothesized changepoint. Specifically, we estimate and compare dispersion on either side of a putative changepoint in variability (Thanksgiving 2020), in order to test the hypothesis that dispersion changes significantly**. 
...
Approximately one month of data on either side of November 26th, 2020 was used in the analysis. This step was done to include a set "wave" of incidence in the model fit to each side of the hypothesized change. However, we acknowledge that there is a trade-off between including fewer waves and increasing the power of the proposed test by including more time series data. We additionally use the full dataset to investigate variability breakpoints using a likelihood ratio test statistic and visualizations of other dispersion metrics. In both the full data and the subset of data, time series that were all zeroes were omitted.

# Methods

## Modeling the proccess mean
...
Issues arise when modeling the entire time series as negative binomial - namely, that autocorrelation remains unaccounted for (Barron 1992). A recently proposed negative binomial regression model for time series of counts also accommodates serial dependence (Davis and Wu 2009). The approach presented here is similar, but we use a linear predictor, $\eta$, that includes a natural spline in time to account for autocorrelation in case counts. Natural splines are cubic splines which are linear outside of the boundary knots (Perperoglou et al. 2019). The degrees of freedom for the spline were chosen to be three, as there appeared to be no more than one knot in a time series of 30 days. In addition, increasing degrees of freedom above three results in overfitting to the time series and thus potentially insignificant changes in dispersion are more readily detected.

![Fitted Curves With Three Spline Degrees of Freedom](figures/FL_signif_fittedcurves.pdf){width=30%, height=30%}


$$g(E[Y_i]/n_i) = \beta_1h_1(t_i) + \beta_2h_2(t_i) + \beta_3h_3(t_i)$$
...
![Diagnostic Plot 2](figures/diagnostic_plot.2.pdf){width=30%, height=30%}

## Proposed Likelihood Ratio Test
...
![Empirical change in dispersion](./figures/empirical_increase.pdf){width=30%, height=30%}
...
The resulting probabilities of rejection are tabulated below: the proportion of tests that reject is around 0.05 for those instances where the null is true. Additionally, the proportion of tests that reject is high in instances where the alternative hypothesis is true. We also simulated data assuming that theta was a function of the mean of the process in order to assess the robustness of the test (Supplement).
...
![Type I Error Rate and Power - LRT Test](figures/pvals_sim_LRT_table.pdf)

![LRT  p-values v. Theta Difference](figures/pvals_sim_LRT.pdf){width=30%, height=30%}

# Results 

## Biogeography of Superspreading Dynamics

For every county, the negative binomial model was fit separately to either side of November 26th, 2020 in order to compute values of $\hat{\theta_1}$ and $\hat{\theta_2}$. A test of the two-sided alternative that $\theta$ before is different from $\theta$ after (different theta after Thanksgiving) was performed.

The spatial pattern of the difference in log(theta) after Thanksgiving 2020 shows that there is spatial correlation in the degree of change. For example, a spatial clusters of increased dispersion occurs in the South. 

![Mapping the Thanksgiving hypothesis across the US (Log Theta)](./figures/map_difftheta_US.pdf){width=30%, height=30%}
...
![LRT Scan Figure](./figures/LRT_scan_figure.pdf){width=30%, height=30%}

Visualizations surfaces of the likelihood ratio test statistic and associated direction of the change in theta for large populations are presented below.

![The LRT Surface](./figures/lrt_surface_figure.pdf){width=30%, height=30%}


![The Theta Difference Surface](./figures/thetadiff_surface.pdf){width=30%, height=30%}

## Patterns in dispersion over time

In many US counties after late fall of 2020, there was more burstiness (smaller log theta) in COVID-19 case counts over time. In other words, cases fluctuated more around a time-averaged trajectory later on, possibly indicating structure in the population with some groups experiencing different infection rates (overdispersed individual reproduction number), demographic/environmental stochasticity, or fast changes in propagation/reporting at the population level. I will investigate whether the trend of increased burstiness over this period is modulated by factors such population size. I predict that in very large populations, dynamics may be "smoother", but this may not be the case in other populations, where more burstiness might be a result of cases "stuttering out" or dispersion at the individual level affecting case count time series. This research avenue has practical implications: smoother and more predictable case count time series may require less surge capacity in hospitals.

We hypothesized that there would be an increase in burstiness (decrease in theta) around Thanksgiving 2020 due to people gathering and causing clusters of infection. However, we observed that in many counties, this was not the case. People may have avoided gathering due to personal preference, cultural norms, and other factors, contributing to less superspreading than anticipated in some locations. 

![Compare](./figures/compare1.pdf){width=30%, height=30%}
