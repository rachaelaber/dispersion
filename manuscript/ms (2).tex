...
We model broad-scale change in incidence as $log(E[I_t / N])  = \sum_j \beta_j h_j(t)$. A persistent challenge in investigating changes in variability has been ``spurious correlation'' with population size. 
...
There is an offset term in order to directly model counts (here, COVID-19 cases) per unit of observation (here, per individual). Let $I_i$ be the number of infections at time step i and $h_i$

\begin{align}
  \log(E[I]/N) &= \beta_1h_1(t_i) + \beta_2h_2(t_i) + \beta_3h_3(t_i) \\
  \log(E[I])-\log(N) &= \beta_1h_1(t_i) + \beta_2h_2(t_i) + \beta_3h_3(t_i) \\ 
  \log(E[I_i]) &= \beta_1 h_1(t_i) + \beta_2h_2(t_i) + \beta_3 h_3(t_i) + log(n_i) 
\end{align}
