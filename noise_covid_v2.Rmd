...
![Fitted Curves With Three Spline Degrees of Freedom](figures/FL_signif_fittedcurves.pdf)
...
length(which(disp_stats > 1))/length(disp_stats)
```

Directly comparing between the quasi-Poisson and the Poisson models via deviance residuals is not possible because the deviance residuals are identical. Therefore, to evaluate whether the negative binomial conditional distribution is needed (opposed to a Poisson/quasi-Poisson conditional distribution with the same model of process mean), inspection of mean-variance relationships using a diagnostic plot is possible (Ver Hoef and Boveng 2007). In the quasi-Poisson model (called the linear negative binomial specification by Barron (1992)), the variance is a linear function of the mean, with a slope of one representing the Poisson special case. By contrast, the negative binomial model may more appropriately represent the data when the variance has approximately a quadratic relationship with the mean. Since the true mean and variance of the data set is unknown, one could approximate these quantities with estimates $\hat{\mu}$ by and $\widehat{\sigma^2}$, respectively. These quantities should be estimated using both the Poisson and the negative binomial model. Variance can be estimated by averaging squared residuals in each fitted mean category. The number of fitted mean categories (bins) should be chosen such that there are an adequate number of observations informing each variance estimate, and also an adequate number of variance estimates to visualize whether a trend is present. Visualization of individual squared residuals is also possible, but these may not be reliable estimates of the variance.

We use a diagnostic plot approach in which we rely on the expected value of the residuals to infer "p" using the slope of a regression of log(Y) on log(X), where X is r^2 - Y. The expectation of X is phi * mu^p, so
 log(E(X)) ~ log(phi) + p log(E(Y))
 
We use a second diagnostic plot, squared residuals versus squared fitted values to look for a linear trend in the Florida counties with a significant LRT test result for the Thanksgiving hypothesis. If the constant dispersion model (NB2) model is ok, then we should see a linear trend. If that checks out, then we don’t have to consider more complicated models.

![Diagnostic Plot](figures/diagnostic_plot.pdf)

![Diagnostic Plot 2](figures/diagnostic_plot.2.pdf)

## Proposed hypothesis test

We tested the hypothesis that the incidence dispersion parameter changes at the Thanksgiving changepoint of interest: $\theta_1 \ne \theta_2$. Hypothesis tests were conducted to assess whether $\theta$ changes after Thanksgiving 2020. The Wald test was used, where the estimates of $\theta$ in different segments are considered independent. This approach is possible because we used IRLS estimates of $\theta$ - the standard error of $\theta$ was found utilizing the observed information matrix. 

![Empirical change in dispersion](./figures/empirical_increase.pdf)
...
For every county, the model was fit separately to either side of November 26th, 2020 in order to compute values of $\hat{\theta_1}$ and $\hat{\theta_2}$. A test of the two-sided alternative that $\theta$ before is different from theta after (different theta after Thanksgiving) was performed.
...
![Wald Test p-values v. Theta Difference](figures/pvals_sim_Wald.pdf)
...
![LRT  p-values v. Theta Difference](figures/pvals_sim_LRT.pdf)

## Biogeography of Superspreading Dynamics

![Mapping the Thanksgiving hypothesis across the US](./figures/map_difftheta_US.pdf)

![LRT Scan Figure](./figures/LRT_scan_figure.pdf)

![The LRT Surface](./figures/lrt_surface_figure.pdf)

![The Theta Difference Surface](./figures/thetadiff_surface.pdf)
