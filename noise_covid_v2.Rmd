...
NY_sig <- (which(pvals < 0.05))

state <- "FL"
source("code/lrt_counties.R")
FL_sig <- (which(pvals < 0.05))
```
Note that in these visualizations, an NB2 fit would be:
A Poisson fit would be:
```{r}
signif_index <- c(NY_sig, FL_sig)
...
source("code/vis_signific_state.R")
```
...
```{r}
load("data/pvals_sim_Wald.Rdata")
mean(pvals[which(curve_parms$theta1==curve_parms$theta2)], col=1)
mean(pvals[which(curve_parms$theta1!=curve_parms$theta2)], col=2)
