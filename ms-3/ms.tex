...
Under a wide range of configurations for a branching process model the number of infected individuals $I_t$ at time $t$ will have a negative binomial distribution~\citep{kendall1949stochastic, grenfell2002dynamics}, $I_t \sim NB \left( \mu_t, \theta_t \right)$, where $\mu_t$ is the expectation for $I_t$ and $\theta_t$ is the dispersion parameter. 
...
Consequently, in order to estimate incidence volatility we first model incidence at broad spatiotemporal scales using natural splines~\citep{perperoglou2019review} fitted within a moving window
\begin{equation}
...
Specifically, since population size influences the mean and variance of case count data, it impacts estimates of dispersion in populations of different sizes that are otherwise identical.
...
We estimate $\theta_t$ given observed incidence using an iteratively reweighted least-squares procedure implemented via the NBPSeq R package~\citep{di2015nbpseq}. 
For each time window, a series of $\mu_s$ from $s = t-\Delta$ to $s = t + \Delta$ is estimated using the spline, and a single value of $\theta_t$ is fitted. [more detail needed]

In addition to fitting the model at each time step, we developed a likelihood-ratio test (LRT) that can be applied at each time step to test the hypothesis that $\theta$ has changed. This involves fitting and comparing both a null model (no \begin{math}\theta\end{math} change) and a two-part (\begin{math}\theta\end{math}-change) model. [more detail needed]

\subsection*{Application to simulated data}

To test the validity and power of the methods, we epidemic curves with known values of $\mu$ and $\theta$.
We varyied the magnitude of the $\theta$ change, location of the change in the curve, population size underlying the curve, and curve shape to test the validity and power of our approach across a range of scenarios.
...
Stochastic noise was then added to these templates by sampling from a negative binomial distribution with mean given by the template values.Each curve was split into two equal pieces and separate dispersion parameters $\theta_1$ and $theta_2$ were used in each respective piece, before and after the breakpoint. 
...
We applied our method to cumulative COVID-19 case countes in US counties~\citep{nytimes_covid19}.
Population sizes were from The United States Census Bureau~\citep{us_census_county_population_2021}.
